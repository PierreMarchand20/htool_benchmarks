...
class LaplaceLikeGenerator : public VirtualGenerator<double> {
  ...
  public:
    ...
    double get_coef(const int &i, const int &j) const {
        return 1. / (1e-5 + std::sqrt(std::inner_product(m_target_points.begin() + m_space_dimension * i, this->m_target_points.begin() + m_space_dimension * i + m_space_dimension, m_source_points.begin() + m_space_dimension * j, double(0), std::plus<double>(), [](double u, double v) { return (u - v) * (u - v); })));
    }
};
} // namespace htool_benchmark
