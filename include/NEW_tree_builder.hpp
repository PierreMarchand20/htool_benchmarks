...
class HMatrixTaskBasedTreeBuilder {
  ...
  public:
    ...
    HMatrixTaskBasedTreeBuilder(const HMatrixTaskBasedTreeBuilder &)                = delete;
    HMatrixTaskBasedTreeBuilder &operator=(const HMatrixTaskBasedTreeBuilder &)     = delete;
    HMatrixTaskBasedTreeBuilder(HMatrixTaskBasedTreeBuilder &&) noexcept            = default;
    HMatrixTaskBasedTreeBuilder &operator=(HMatrixTaskBasedTreeBuilder &&) noexcept = default;
...
template <typename CoefficientPrecision, typename CoordinatePrecision>
bool HMatrixTaskBasedTreeBuilder<CoefficientPrecision, CoordinatePrecision>::build_block_tree(HMatrixType *current_hmatrix) const {
...
template <typename CoefficientPrecision, typename CoordinatePrecision>
void HMatrixTaskBasedTreeBuilder<CoefficientPrecision, CoordinatePrecision>::reset_root_of_block_tree(HMatrixType &root_hmatrix) const {
...
#    pragma omp single
        ...
        {
#if defined(_OPENMP) && !defined(PYTHON_INTERFACE)
#    pragma omp taskloop default(none) shared(local_false_positive, generator)
#endif
            for (int p = 0; p < m_admissible_tasks.size(); p++) {
                m_admissible_tasks[p]->compute_low_rank_data(generator, *m_low_rank_generator, m_reqrank, m_epsilon); // modifie m_low_rank_data et m_storage_type
                // local_low_rank_leaves.emplace_back(m_admissible_tasks[p]);
                if (m_admissible_tasks[p]->get_low_rank_data()->rank_of() == -1) { // ie le cas où ce n'est pas conseillé de compresser
                    // local_low_rank_leaves.pop_back();
                    m_admissible_tasks[p]->clear_low_rank_data(); // Do m_low_rank_data.reset()
                    // if (m_dense_blocks_generator.get() == nullptr) {
                    m_admissible_tasks[p]->compute_dense_data(generator); // modifie m_dense_data, m_storage_type et generator
                    // }
